...
def get_shape_metadata_from_dataset(dataset_path, all_obs_keys=None, verbose=False, ac_key="actions"):
    ...
    for k in sorted(all_obs_keys):
        try:
            ...
            # Handle missing audio data - use predefined shape for audio
            if k == "audio":
                initial_shape = (32000,)  # Raw audio shape for 2 seconds at 16kHz
                if verbose:
                    print("obs key {} with predefined shape {} (not found in dataset)".format(k, initial_shape))
            else:
                raise  # Re-raise for other missing keys
