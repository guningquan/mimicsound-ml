...
from mimicsound.utils.mimicsoundUtils import nds, cam_frame_to_cam_pixels,REALSENSE_INTRINSICS, draw_dot_on_frame
import argparse
import torch
import numpy as np
import h5py
...
import cv2
from mimicsound.scripts.masking.utils import Inpainter, inpaint_hdf5, SAM
    ...
    with h5py.File(dataset, "r+") as data:
        with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
            for i in tqdm(range(len(data["data"].keys()))):
                ...
                overlayed_imgs, masked_imgs, raw_masks = sam.get_hand_mask_line_batched(imgs, ee_poses, REALSENSE_INTRINSICS, debug=debug)
