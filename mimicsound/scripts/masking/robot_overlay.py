...
import cv2
from mimicsound.utils.mimicsoundUtils import nds, ee_pose_to_cam_frame, EXTRINSICS, REALSENSE_INTRINSICS, cam_frame_to_cam_pixels
...
def main(args):
    ...
    arm = args.arm
    chain = pk.build_serial_chain_from_urdf(open("/mimicsound/resources/model.urdf").read(), "vx300s/ee_gripper_link")
    ...
    with h5py.File(args.dataset, 'r+') as aloha_hdf5, torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
        ...
        # for j in tqdm(range(0, 1)):
            ...
            px_dict = sam.project_joint_positions_to_image(torch.from_numpy(aloha_hdf5[f'data/demo_{j}/obs/joint_positions'][:, :]), EXTRINSICS[args.extrinsics], REALSENSE_INTRINSICS, arm=arm)
