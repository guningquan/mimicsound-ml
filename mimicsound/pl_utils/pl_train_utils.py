...
from mimicsound.utils.train_utils import get_exp_dir, load_data_for_training
import copy
from mimicsound.utils.mimicsoundUtils import nds
from mimicsound.pl_utils.pl_model import ModelWrapper
from mimicsound.pl_utils.pl_data_utils import (
    DataModuleWrapper,
    ...
    def handle_preemption(self, signum, frame):
        if self.trainer_ref is not None:
            if self.trainer_ref.global_rank == 0:
                ...
                abs_last_path = os.path.join(mimicsound.__path__[0], last_path)
                abs_path = os.path.join(mimicsound.__path__[0], path)
