...
from mimicsound.utils.dataset import PlaydataSequenceDataset
from robomimic.envs.env_base import EnvBase
from robomimic.envs.wrappers import EnvWrapper

import mimicsound
from mimicsound.algo import RolloutPolicy
...
def get_exp_dir(config, auto_remove_exp_dir=False, rank=0):
    ...
    if not os.path.isabs(base_output_dir):
        ...
        base_output_dir = os.path.join(mimicsound.__path__[0], base_output_dir)
...
def load_data_for_training(config, obs_keys, type, dataset_path=None):
    ...
    if config.experiment.validate:
        ...
        # assert set(train_demo_keys).isdisjoint(set(valid_demo_keys)), (  # gnq
