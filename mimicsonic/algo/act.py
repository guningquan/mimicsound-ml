...
class ACTModel(nn.Module):
    ...
    def _forward(self, qpos, actions, image, encoder_action_proj=None, encoder_joint_proj=None, transformer_input_proj=None, action_head=None, camera_names=None, env_state=None, is_pad=None, aux_action_head=None):
        ...
        # print("len(camera_names) is", len(camera_names)) # gnq
...
class ACT(BC_VAE):
    ...
    def _robomimic_to_act_data(self, batch, cam_keys, proprio_keys):
        ...
        # print("cam_keys is", cam_keys) # @gnq
        ...
        if len(cam_keys) > 0:
            for cam_name in cam_keys:
                ...
                # # Debug: Display image for temporary debugging
                # import matplotlib.pyplot as plt
                # import numpy as np
                ...
                # if len(img_np.shape) == 4:  # (B, C, H, W) or (B, H, W, C)
                #     img_np = img_np[0]  # Take first batch
                # elif len(img_np.shape) == 3:  # (C, H, W) or (H, W, C)
                #     img_np = img_np
                # else:
                #     continue
                ...
                # plt.axis('off')
                # plt.show()
                # print(f"Displayed image from camera: {cam_name}, shape: {img_np.shape}")
