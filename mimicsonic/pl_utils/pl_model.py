...
import mimicsonic.utils.val_utils as ValUtils
from mimicsonic.utils.mimicsonicUtils import nds
from mimicsonic.pl_utils.pl_data_utils import DualDataModuleWrapper, json_to_config
from mimicsonic.algo import algo_factory
...
class ModelWrapper(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # for key in loss_dicts[0].keys():
        #     print("key is", key)
        #     stacked = torch.stack([loss_dict[key] for loss_dict in loss_dicts])
        #     print("values before mean:", stacked)   # <-- print the tensor values
        #     losses[key] = torch.mean(stacked)
    ...
    def custom_eval(self, video_dir, type=None):
        self.eval()
        self.zero_grad()
        # Determine type from config if not provided
        if type is None:
            type = getattr(self.model.global_config.train, 'data_type', 'robot')
        with torch.no_grad():
