...
from mimicsonic.utils.train_utils import get_exp_dir, load_data_for_training
import copy
from mimicsonic.utils.mimicsonicUtils import nds
from mimicsonic.pl_utils.pl_model import ModelWrapper
from mimicsonic.pl_utils.pl_data_utils import (
    DataModuleWrapper,
    ...
    def handle_preemption(self, signum, frame):
        if self.trainer_ref is not None:
            if self.trainer_ref.global_rank == 0:
                ...
                abs_last_path = os.path.join(mimicsonic.__path__[0], last_path)
                abs_path = os.path.join(mimicsonic.__path__[0], path)
...
def eval(config, ckpt_path, type):
    ...
    step_log = model.custom_eval(video_dir, type=type)
