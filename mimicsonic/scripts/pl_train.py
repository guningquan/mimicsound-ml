...
import os
import traceback
from mimicsonic.configs import config_factory
import datetime
import time
from mimicsonic.utils.mimicsonicUtils import nds
import matplotlib.pyplot as plt
from mimicsonic.pl_utils.pl_train_utils import train, eval
from mimicsonic.pl_utils.pl_data_utils import json_to_config
import torch
import numpy as np
import mimicsonic
...
def main(args):
    ...
    if args.config is not None:
        ...
        print("=== Before config.update() ===")
        try:
            print(f"audio_enabled: {config.audio_enabled}")
        except (AttributeError, RuntimeError):
            print("audio_enabled: NOT_FOUND")
        try:
            print(f"audio_config: {config.audio_config}")
        except (AttributeError, RuntimeError):
            print("audio_config: NOT_FOUND")
        print(f"ext_cfg audio_enabled: {ext_cfg.get('audio_enabled', 'NOT_FOUND')}")
    ...
    if args.name is not None and args.description is not None:
        base_output_dir = os.path.dirname(mimicsonic.__file__)
    ...
    if args.dataset is not None:
        ...
        # If data_type is not set, default to 'robot' for main dataset
        if not hasattr(config.train, 'data_type') or config.train.data_type is None:
            config.train.data_type = 'robot'

    if args.dataset_2 is not None:
        config.train.data_2 = args.dataset_2
        # If data_2_type is not set, default to 'hand' for second dataset
        if not hasattr(config.train, 'data_2_type') or config.train.data_2_type is None:
            config.train.data_2_type = 'hand'
    ...
    # Handle audio modal configuration
    # Only override audio settings if explicitly specified via command line
    if hasattr(args, 'audio_enabled') and args.audio_enabled:
        config.audio_enabled = True
        if hasattr(args, 'audio_debug') and args.audio_debug:
            config.audio_debug = True
        else:
            config.audio_debug = False
    # Don't override config.audio_enabled if not explicitly specified via command line
    # This allows the JSON config to set audio_enabled = true
    ...
    try:
        if args.eval:
            # Ensure data_type is set for evaluation
            eval_type = config.train.data_type if hasattr(config.train, 'data_type') and config.train.data_type is not None else 'robot'
            if eval_type not in ['robot', 'hand']:
                # Try to infer from dataset path
                dataset_path = config.train.data if hasattr(config.train, 'data') else None
                if dataset_path and 'human' in dataset_path.lower():
                    eval_type = 'hand'
                else:
                    eval_type = 'robot'
            print(f"Using data_type: {eval_type} for evaluation")
            eval(config, args.ckpt_path, type=eval_type)
            return
        else:
...
def train_argparse():
    ...
    # Audio modal arguments
    parser.add_argument(
        "--audio-enabled", action="store_true", help="Enable audio modal support"
    )
    
    parser.add_argument(
        "--audio-debug", action="store_true", help="Use random audio data for debugging"
    )
