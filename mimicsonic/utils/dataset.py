...
from mimicsonic.utils.audio_utils import generate_audio_data_for_batch
...
class PlaydataSequenceDataset(SequenceDataset):
    ...
    def get_item(self, index):
        ...
        # Try to load all obs_keys first, including mel_spectrogram if it exists
        try:
            ...
            # print("using collected audio data for obs")
            # print("obs shape is:", meta["obs"]["audio"].shape)
            # exit()
        except KeyError as e:
            if "audio" in str(e):
                # If audio doesn't exist in dataset, load other keys and generate audio data
                # print("using random audio data for obs")
                # exit()
                filtered_obs_keys = [k for k in self.obs_keys if k != "audio"]
                meta["obs"] = self.get_obs_sequence_from_demo(
                    demo_id,
                    index_in_demo=index_in_demo,
                    keys=filtered_obs_keys,
                    num_frames_to_stack=self.n_frame_stack - 1,
                    seq_length=self.seq_length,
                    prefix="obs",
                    dont_load_fut=self.rgb_keys,
                ...
                meta["obs"]["audio"] = generate_audio_data_for_batch(
                    batch_size=1,
                    seq_length=self.seq_length,
                    device=None,  # Will be moved to device later
                    debug_mode=True
                ).squeeze(0)  # Remove batch dimension since we're generating for single sample
            else:
        ...
        if self.load_next_obs:
            try:
                ...
                if "audio" in str(e):
                    ...
                    # Generate random audio data
                    meta["next_obs"]["audio"] = generate_audio_data_for_batch(
                        batch_size=1,
                        seq_length=self.seq_length,
                        device=None,  # Will be moved to device later
                        debug_mode=True
        ...
        if goal_index is not None:
            try:
                ...
                if "audio" in str(e):
                    ...
                    # Generate random audio data
                    meta["goal_obs"]["audio"] = generate_audio_data_for_batch(
                        batch_size=1,
                        seq_length=self.seq_length,
                        device=None,  # Will be moved to device later
                        debug_mode=True
